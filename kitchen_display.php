...
try {
    ...
    $st_store = $pdo->prepare("SELECT kds_theme, kds_kitchen_theme, kds_sync_order_status, kds_datetime_locale, kds_alert_5, kds_alert_10, kds_alert_20, kds_alert_30, store_name FROM stores WHERE id = ?");
    ...
    if (isset($_GET['complete_order'])) {
        ...
        if (!$row || (int)$row['store_id'] !== $store_id) {
            ...
            if ($kds_sync_order_status === 1) {
                $upd = $pdo->prepare("UPDATE orders SET status = 'SERVED' WHERE id = ? AND store_id = ?");
    ...
    // 3. 해당 가맹점 주문 중 미완료(미서빙) 항목만 조회 (orders 테이블만 사용, session_id 없음)
    ...
    // 주문 히스토리: 해당 매장의 주문 중, 모든 order_items 가 'served' 인 주문만 최근 30건 표시
    // 각 주문에 대해 주문 번호 + 메뉴/수량 요약을 함께 보여준다.
    $sth = $pdo->prepare("
        SELECT 
            o.id,
            o.created_at,
                        ...
                        <div class="<?php echo $kitchen_theme['card']; ?> rounded-2xl overflow-hidden shadow-2xl <?php echo $kitchen_theme['cardBorder']; ?> <?php echo $ageClass; ?>">
                            <div class="<?php echo $kitchen_theme['cardHeader']; ?> p-4 flex justify-between items-center">
                                <div>
                                    ...
                                    <span class="ml-2 text-xs font-mono <?php echo $kitchen_theme['textMuted']; ?>"><?php echo $diffMin; ?>분 경과</span>
                                </div>
                    ...
                    <ul class="space-y-2 max-h-[70vh] overflow-y-auto">
                        <?php foreach ($history_served as $h): ?>
                            <li class="text-xs <?php echo $kitchen_theme['textMuted']; ?> flex justify-between items-center gap-2 border-t border-slate-200 first:border-t-0 pt-2 mt-2 first:pt-0 first:mt-0">
                                <div class="flex-1 min-w-0">
                                    <?php
                                        // 현재 DB에는 table_no 컬럼이 없으므로 테이블 표시는 생략하고 시간만 사용
                                        $tbl = '-';

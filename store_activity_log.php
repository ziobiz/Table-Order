<?php
// store_activity_log.php - 가맹점: 로그 분석(변경 이력) · 14일 보관 후 자동 삭제
...
$store_retention_days = get_activity_log_retention_days($pdo, 'store');
// 설정된 일수 초과 가맹점 로그 자동 삭제 (통합설정 · 일괄 적용)
try {
    $pdo->exec("DELETE FROM activity_logs WHERE scope = 'store' AND created_at < DATE_SUB(NOW(), INTERVAL " . (int)$store_retention_days . " DAY)");
} catch (Exception $e) {}
...
$where = "scope = 'store' AND actor_id = ?";
$params = [$store_id];
if ($page_filter !== '') {
    $where .= " AND page = ?";
    $params[] = $page_filter;
...
$pages = [];
try {
    $stmt = $pdo->prepare("SELECT DISTINCT page FROM activity_logs WHERE scope = 'store' AND actor_id = ? ORDER BY page");
    ...
    <div class="max-w-[96rem] mx-auto space-y-6">
        <header class="flex flex-wrap items-center justify-between gap-3">
            <div>
                ...
                <p class="text-xs text-slate-500 mt-1">가맹점 변경 이력 · <?php echo (int)$store_retention_days; ?>일 보관 후 자동 삭제 (통합설정) · <?php echo htmlspecialchars($store_name); ?></p>
            </div>
            <a href="store_dashboard.php" class="bg-white border-2 border-slate-200 px-5 py-2.5 rounded-2xl text-xs font-black uppercase hover:bg-slate-50">Back to Dashboard</a>
        </header>
        ...
        <div class="space-y-8">
            ...
            <div class="bg-white rounded-[2rem] shadow-lg border border-slate-100 overflow-hidden">
                <div class="px-5 py-4 bg-slate-100 border-b border-slate-200 flex flex-wrap items-center justify-between gap-2">
                    ...
                    <span class="text-[10px] text-slate-500"><?php echo count($rows); ?>건 · 보관 <?php echo (int)$store_retention_days; ?>일</span>
                </div>
                    ...
                    <table class="w-full text-left text-sm">
                        ...
                        <tbody>
                            ...
                            <tr class="border-b border-slate-50 hover:bg-slate-50/50">
                                ...
                                <td class="py-3 px-3 font-bold text-slate-800"><?php echo htmlspecialchars($row['actor_name']); ?></td>
